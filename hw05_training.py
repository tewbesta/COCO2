...
a,b,c=dataserver_train[1]
#total=len(dataserver_train)


#def load_coco(self, dataserver_train, dataserver_val):
    #            transform = tvt.Compose([tvt.ToTensor(),
    #                                tvt.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
train_dataloader = torch.utils.data.DataLoader(dataserver_train,
                                                        batch_size=10, shuffle=True,
                                                        num_workers=0)
#val_dataloader = torch.utils.data.DataLoader(dataserver_val,batch_size=1, shuffle=False, num_workers=0)
...
def train_epocs(model, train_dl, epoch=10,C=1000):
    ...
    for i in range(epoch):
      ...
      for i, data in enumerate(train_dl):
            ...
            print("........................",y_class.shape,out_class.shape)
            ...
            if i % 100 == 0:
                avg_loss_labeling = running_loss_labeling / float(100)
                avg_loss_regression = running_loss_regression / float(100)
            ...
            print("the batch of ybb;;;;;;;;;",len(y_bb))
            print("the len of ybb;;;;;;;;;", len(y_bb[0]))
            for b in range(len(y_bb)):
              for s in range(len(y_bb[0])):
                if y_bb[b][s]==out_bb[b][s]:
                    count+=1
      print("the accuracy is ",count/(i+1)*4)

    torch.save(model.state_dict(), os.path.join(args.root_path + "trainon.pth"))
    plt.figure()
    plt.plot(loss1, color='blue', label="train")
    plt.show()
    plt.plot(loss2, color='orange', label="train")
    plt.show()
BB_model=model.BB_model()


train_epocs(BB_model,train_dataloader,epoch=30,C=args.images_per_class)
